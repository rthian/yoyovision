...
import { getAnalysis, getScore, recomputeScore } from "@/lib/api-client";
...
/** Invalidates the score query for `analysisId`; every event/deduction/
...
export function useInvalidateScore(analysisId: string) {
  const queryClient = useQueryClient();
  return () => queryClient.invalidateQueries({ queryKey: scoreQueryKey(analysisId) });
}
